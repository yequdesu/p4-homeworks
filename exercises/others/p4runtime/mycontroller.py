    ...
    for response in sw.ReadTableEntries():
        for entity in response.entities:
            ...
            for m in entry.match:
                field = p4info_helper.get_match_field_name(table_name, m.field_id)
                value = p4info_helper.get_match_field_value(m)
                if "ipv4.dstAddr" in field and isinstance(value, tuple):
                    ip_bytes, prefix = value
                    ip = ".".join(str(b) for b in ip_bytes)
                    value = f"{ip}/{prefix}"
                elif "myTunnel.dst_id" in field and isinstance(value, bytes):
                    tunnel_id = int.from_bytes(value, 'big')
                    value = f"{tunnel_id} (0x{tunnel_id:04x})"
            ...
            for p in action.params:
                param = p4info_helper.get_action_param_name(action_name, p.param_id)
                value = p.value
                if "dstAddr" in param and len(value) == 6:
                    mac = ":".join(f"{b:02x}" for b in value)
                    value = mac
                elif "port" in param and isinstance(value, bytes):
                    value = int.from_bytes(value, 'big')
                elif "dst_id" in param and isinstance(value, bytes):
                    tunnel_id = int.from_bytes(value, 'big')
                    value = f"{tunnel_id} (0x{tunnel_id:04x})"
                print(f"  参数: {param:<20} = {value}")
